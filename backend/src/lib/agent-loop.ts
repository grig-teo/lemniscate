...
// (LLM review → fix iterations), 'merge-gate' jobs (CI-gated auto-merge with
// CI fix and conflict resolution), and 'generate-proposals' jobs (LLM
// suggests up to 5 improvement tasks).
//
// The implementation is split into focused modules (AGENTS.md §2):
//   agent-run.ts        'run-task' job
//   agent-review.ts     'review-pr' job
//   merge-gate.ts       'merge-gate' job
