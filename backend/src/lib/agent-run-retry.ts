...
export async function handleNoChangesProduced(taskId: string, attempt: number): Promise<RunOutcome> {
  if (attempt < NO_CHANGE_MAX_ATTEMPTS) {
    await requeueForRetry(taskId, attempt);
    return 'retry';
  }
  return failNoChanges(taskId, attempt);
}
...
export async function taskStillClaimable(
  taskId: string,
