...
- **gitstub** (`tests/e2e/gitstub/`): a single container serving a bare git
  repo over HTTPS (the "git provider", via the existing `bare` git provider)
  and a deterministic OpenAI-compatible stub LLM whose canned change-set
  response comes from `llm-fixture.json`. No external secrets or network
  access are needed.
