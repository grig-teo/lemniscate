 ...
 * hide/show toggle persisted to localStorage. Hiding removes the panel
 * entirely — no "Plan" show handle or icon remains on the right edge.
 */
...
describe('ObjectiveTodoPanel', () => {
  ...
  it('hides via the toggle without leaving a Plan show handle or icon', () => {
    render(<ObjectiveTodoPanel objective="x" todoItems={ITEMS} />);
    fireEvent.click(screen.getByRole('button', { name: /hide panel/i }));
    // Panel fully hidden — no "Plan" button, no show handle, no icon.
  ...
  it('shows the panel again when the persisted hidden preference is cleared', () => {
    window.localStorage.setItem('lemniscate.objective-todo-hidden', 'true');
    const view = render(<ObjectiveTodoPanel objective="x" todoItems={ITEMS} />);
    expect(screen.queryByText('Set up project')).toBeNull();
    window.localStorage.removeItem('lemniscate.objective-todo-hidden');
    view.unmount();
    render(<ObjectiveTodoPanel objective="x" todoItems={ITEMS} />);
    expect(screen.getByText('Set up project')).toBeTruthy();
  });
